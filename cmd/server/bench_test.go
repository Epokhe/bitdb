...
func Benchmark_RPC_Get(b *testing.B) {
	...
	addr, cleanup, err := remote.StartRPC(db, ":1234")
	if err != nil {
	...
	for i := 0; i < 10000; i++ {
		...
		client.Call("DB.Set", &remote.SetArgs{Key: key, Val: "v"}, new(struct{}))
	}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		client.Call("DB.Get", &remote.GetArgs{Key: "k9950"}, new(string))
	}
}
...
func Benchmark_RPC_Set(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		if err := client.Call("DB.Set", &remote.SetArgs{Key: key, Val: "value"}, new(struct{})); err != nil {
